...
$(document).ready(function () {
  ...
  function displayNotes(filter) {
    ...
    var content =
      "<table class='table table-bordered table-striped'><thead><tr><th>Название</th><th>Обновлено</th><th>&nbsp;</td></thead><tbody>";
    ...
    var handleResult = function (event) {
      ...
      if (cursor) {
        ...
        content +=
          '<td><a class="btn btn-primary edit">Изменить</a> <a class="btn btn-danger delete">Удалить</a></td>';
        content += "</tr>";
        cursor.continue();
      } else {
        content += "</tbody></table>";
  ...
  function doCount() {
    db
      ...
      $("#sizeSpan").text("(" + event.target.result + " записей всего)");
    };
  }
  ...
  $("#noteList").on("click", "a.edit", function (e) {
    ...
    request.onsuccess = function (event) {
      ...
      $("#title1").val(note.title1);
      $("#size").val(note.size);
      $("#weight").val(note.weight);
      $("#type").val(note.type);
      $("#date").val(note.date);
      $("#body").val(note.body);
      $("#photo1").val(note.photo1);
    ...
    request.onsuccess = function (event) {
      ...
      if (note.tags.length > 0) {
        ...
        note.tags.forEach(function (elm, idx, arr) {
          content +=
            "<a class='tagLookup' title='Click for Related Notes' data-noteid='" +
            note.id +
            "'> " +
            elm +
            "</a> ";
      ...
      content += "<p>" + note.title1 + "</p>";
      content += "<p>" + note.size + "</p>";
      content += "<p>" + note.weight + "</p>";
      content += "<p>" + note.type + "</p>";
      content += "<p>" + note.date + "</p>";
      content += "<p>" + note.body + "</p>";
      content += "<p>" + note.photo1 + "</p>";
